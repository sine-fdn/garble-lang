...
use convert::{bristol_to_garble, garble_to_bristol, ConverterError};
...
/// Scans, parses, type-checks and then compiles the `"main"` fn of a program to a Boolean circuit.
/// The circuit is then converted to the Bristol format and written to the specified path.
...
/// Converts a circuit in Bristol format to a Garble circuit.
/// This is useful for converting circuits generated by other tools or libraries
/// that support the Bristol format to a Garble circuit that can be used with, e.g., Polytune.
pub fn compile_bristol_to_circuit(path: &Path) -> Result<Circuit, Error> {
    Ok(bristol_to_garble(path)?)
}
