...
impl Type {
    pub(crate) fn size_in_bits_for_defs(&self, prg: &Program) -> usize {
        match self {
            Type::Bool => 1,
...
pub(crate) fn struct_size(struct_def: &StructDef, prg: &Program) -> usize {
...
pub(crate) fn enum_max_size(enum_def: &EnumDef, prg: &Program) -> usize {
    let mut max = 0;
