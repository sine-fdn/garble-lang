...
#[test]
fn smart_cookie_interaction() -> Result<(), Error> {
    ...
    for interest in ["Politics", "Politics", "Cars", "Luxury", "Cars", "Cars"] {
        ...
        let log_interest_result = log_interest_eval
            ...
            .map_err(|e| pretty_print(e, &log_interest))?;
        user_state = match &log_interest_result {
            Literal::Enum(enum_name, variant_name, fields) => {
                match (enum_name.as_str(), variant_name.as_str()) {
                    ("LogInterestResult", "Ok") => match fields {
                        VariantLiteral::Tuple(fields) if fields.len() == 2 => {
                            Literal::Tuple(fields.clone())
                        }
                        _ => {
                            panic!("Expected a variant with 2 fields, found {fields:?}")
                        }
                    },
                    _ => {
                        panic!("Unexpected enum: {log_interest_result}")
                    }
                }
            }
            result => panic!("Expected an enum result, but found {result}"),
        };
    }
    ...
    match &ad_decision {
        Literal::Enum(enum_name, variant_name, fields) => {
            match (enum_name.as_str(), variant_name.as_str()) {
                ("AdDecisionResult", "Ok") => match fields {
                    VariantLiteral::Tuple(fields) if fields.len() == 1 => {
                        assert_eq!(
                            fields[0],
                            Literal::Enum(
                                "UserInterest".to_string(),
                    ...
                    _ => {
                        panic!("Expected a variant with 1 field, found {fields:?}")
                    }
                },
                _ => {
                    panic!("Unexpected enum: {ad_decision}");
                }
            }
        }
        _ => panic!("Unexpected ad decision result: {ad_decision}"),
    }
